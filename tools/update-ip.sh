...
if [ "$DRY_RUN" = true ]; then
    ...
    echo "DRY RUN: Would restart Core Registry service"
else
    ...
    # Restart the Core Registry service
    echo "Restarting Core Registry service..."
    SERVICE_NAME="core-registry-api@${CHIA_USER}.service"
    if systemctl is-active --quiet "$SERVICE_NAME"; then
        systemctl restart "$SERVICE_NAME"
        echo "Core Registry service restarted successfully"
    else
        echo "Warning: Core Registry service is not running. Skipping restart."
    fi
fi
